...
//get group 0
$sql  = 'SELECT copyrightdate as label,  count(*) as value FROM ' . THIS_TABLE . ' WHERE ' . $sqlstr . ' and lastborrowed is null GROUP BY copyrightdate';
...
//get group 1
$sql  = 'SELECT copyrightdate as label,  count(*) as value FROM ' . THIS_TABLE . ' WHERE ' . $sqlstr . ' and LEFT(lastborrowed,4) >= :group0 and LEFT(lastborrowed,4) <= :group1 GROUP BY copyrightdate';
$stmt = $pdo->prepare($sql);
$stmt->execute(array(
    ':group0' => $group0,
    ':group1' => $group1
...
//get group 2
$sql  = 'SELECT copyrightdate as label,  count(*) as value FROM ' . THIS_TABLE . ' WHERE ' . $sqlstr . ' and LEFT(lastborrowed,4) > :group1 GROUP BY copyrightdate';
$stmt = $pdo->prepare($sql);
$stmt->execute(array(
    ':group1' => $group1
));
...
?>
    ...
    <!--     <h1 class="page-header">All Item by Copyright Date</h1> -->
           ...
           <h2><?php echo $thisauthor . " " . count($allresultsitems) . " items"; ?></h2>  
